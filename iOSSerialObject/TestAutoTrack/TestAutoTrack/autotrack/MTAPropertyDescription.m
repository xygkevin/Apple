...
@implementation MTAPropertySelectorParameterDescription
...
@implementation MTAPropertySelectorDescription
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
	...
	if (self) {
		...
		for (NSDictionary *parameter in dictionary[@"parameters"]) {
			[parameters addObject:[[MTAPropertySelectorParameterDescription alloc] initWithDictionary:parameter]];
		}
...
@interface MTAPropertyDescription ()

@property (nonatomic, readonly) NSPredicate *predicate;

@end

@implementation MTAPropertyDescription
...
+ (NSValueTransformer *)valueTransformerForType:(NSString *)typeName {
	...
	for (NSString *toTypeName in @[@"NSDictionary", @"NSNumber", @"NSString"]) {
		NSString *toTransformerName = [NSString stringWithFormat:@"%@%@To%@ValueTransformer", prefix, typeName, toTypeName];
	...
	return [NSValueTransformer valueTransformerForName:[NSString stringWithFormat:@"%@PassThroughValueTransformer", prefix]];
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
	...
	if (self) {
		...
		_getSelectorDescription = [[MTAPropertySelectorDescription alloc] initWithDictionary:get];
		if (set) {
			_setSelectorDescription = [[MTAPropertySelectorDescription alloc] initWithDictionary:set];
		} else {
			_readonly = YES;
		}
