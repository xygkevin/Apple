...
//#import "NSData+MTABase64.h"
//#import "MTALogger.h"
#import "MTAObjectSelector.h"

@interface MTAObjectFilter : NSObject
...
@implementation MTAObjectSelector

+ (MTAObjectSelector *)objectSelectorWithString:(NSString *)string {
	return [[MTAObjectSelector alloc] initWithString:string];
}
...
- (NSArray *)selectFromRoot:(id)root evaluatingFinalPredicate:(BOOL)finalPredicate {
	...
	if (root) {
		...
		for (NSUInteger i = 0, n = [_filters count]; i < n; i++) {
			MTAObjectFilter *filter = _filters[i];
			...
			if ([_scanner scanInteger:&index] && [_scanner scanCharactersFromSet:_predicateEndChar intoString:nil]) {
				...
				@try {
					if ([predicateFormat isEqualToString:@"()"]) {
						filter.predicate = [NSPredicate predicateWithValue:YES];
					} else {
...
- (Class)selectedClass {
	if ([_filters count] > 0) {
		return NSClassFromString(((MTAObjectFilter *)_filters[[_filters count] - 1]).name);
	}
	return nil;
}
...
@implementation MTAObjectFilter
 ...
 */
- (BOOL)appliesTo:(NSObject *)view {
	return (([self.name isEqualToString:@"*"] || [view isKindOfClass:NSClassFromString(self.name)]) && (self.nameOnly || ((!self.predicate || [_predicate evaluateWithObject:view]) && (!self.index || [self isView:view siblingNumber:[_index integerValue]]) && (!(self.unique) || [self isView:view oneOfNSiblings:1]))));
}
...
- (BOOL)isView:(NSObject *)view siblingNumber:(NSInteger)index of:(NSInteger)numSiblings {
	...
	for (NSObject *parent in parents) {
		if ([parent isKindOfClass:[UIView class]]) {
			...
			if ((index < 0 || ((NSUInteger)index < [siblings count] && siblings[(NSUInteger)index] == view)) && (numSiblings < 0 || [siblings count] == (NSUInteger)numSiblings)) {
				return YES;
			}
		}
	}
