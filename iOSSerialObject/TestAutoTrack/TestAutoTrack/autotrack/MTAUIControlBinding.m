...
@implementation MTAUIControlBinding
...
+ (MTAEventBinding *)bindingWithJSONObject:(NSDictionary *)object {
	...
	if (!(object[@"control_event"] && ([object[@"control_event"] unsignedIntegerValue] & UIControlEventAllEvents))) {
		//        MTAError(@"must supply a valid UIControlEvents value for control_event");
		return nil;
	}

	UIControlEvents verifyEvent = object[@"verify_event"] ? [object[@"verify_event"] unsignedIntegerValue] : 0;
	return [[MTAUIControlBinding alloc] initWithEventId:eventId
...
- (instancetype)initWithEventId:(NSString *)eventId
	...
	if (self = [super initWithEventId:eventId andTriggerId:triggerId onPath:path isDeployed:deployed]) {
		...
		NSUInteger pathEndIndex = [path rangeOfString:@"["].location;
		NSString *applyClass = nil;
		if (pathEndIndex != NSNotFound) {
			applyClass = [[path substringToIndex:pathEndIndex] lastPathComponent];
		} else {
			applyClass = [path lastPathComponent];
		}

		if ([applyClass isEqualToString:@"UISlider"] || [applyClass isEqualToString:@"UISwitch"]) {
			_controlEvent = UIControlEventValueChanged;
		} else if ([applyClass isEqualToString:@"UITextField"]) {
			_controlEvent = UIControlEventEditingDidEnd;
		} else {
			_controlEvent = UIControlEventTouchDown;
		}
...
- (NSString *)description {
	return [NSString stringWithFormat:@"Event Binding: '%@' for '%@'", [self eventId], [self path]];
}
...
- (void)execute {
	if (!self.running) {
			...
			if (view && [self.appliedTo containsObject:view]) {
				...
				for (UIControl *control in objects) {
					if ([control isKindOfClass:[UIControl class]]) {
						...
						//                        MTADebug(@"%@ event binding [%d, %d on '%@' for %@] executed.",
		...
		[MTASwizzler swizzleSelector:NSSelectorFromString(@"didMoveToWindow")
							 onClass:self.swizzleClass
						   withBlock:executeBlock
							   named:self.name];
		[MTASwizzler swizzleSelector:NSSelectorFromString(@"didMoveToSuperview")
							 onClass:self.swizzleClass
						   withBlock:executeBlock
							   named:self.name];
...
- (void)stop {
	if (self.running) {
		...
		[MTASwizzler unswizzleSelector:NSSelectorFromString(@"didMoveToWindow")
							   onClass:self.swizzleClass
								 named:self.name];
		[MTASwizzler unswizzleSelector:NSSelectorFromString(@"didMoveToSuperview")
							   onClass:self.swizzleClass
								 named:self.name];
...
- (void)eventAction:(id)sender forEvent:(UIEvent *)event {
	...
	if (shouldTrack) {
		[self track:[self eventId] withProperties:nil];
	}
}
