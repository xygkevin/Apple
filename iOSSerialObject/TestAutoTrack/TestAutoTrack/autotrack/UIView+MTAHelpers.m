...
- (UIImage *)mta_snapshotForBlur {
	UIImage *image = [self mta_snapshotImage];
...
- (NSString *)mta_controllerVariable {
	NSString *result = nil;
	if ([self isKindOfClass:[UIControl class]]) {
...
static NSString *mta_encryptHelper(id input) {
	NSString *SALT = @"dcba253e6d2cd94bee5da501240b44c1";
...
- (NSString *)mp_varA {
	return mta_encryptHelper([self mta_viewId]);
}

- (NSString *)mp_varB {
	return mta_encryptHelper([self mta_controllerVariable]);
}

- (NSString *)mp_varC {
	return mta_encryptHelper([self mta_imageFingerprint]);
}

- (NSArray *)mp_varSetD {
	NSArray *targetActions = [self mta_targetActions];
	NSMutableArray *encryptedActions = [NSMutableArray array];
	for (NSUInteger i = 0; i < [targetActions count]; i++) {
		[encryptedActions addObject:mta_encryptHelper(targetActions[i])];
	}
	return encryptedActions;
}

- (NSString *)mp_varE {
	return mta_encryptHelper([self mta_text]);
}
