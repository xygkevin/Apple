...
static void mta_swizzledMethod_2(id self, SEL _cmd) {
	Method aMethod = class_getInstanceMethod([self class], _cmd);
	MTASwizzle *swizzle = (MTASwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
	if (swizzle) {
...
static void mta_swizzledMethod_3(id self, SEL _cmd, id arg) {
	Method aMethod = class_getInstanceMethod([self class], _cmd);
	MTASwizzle *swizzle = (MTASwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
	if (swizzle) {
...
static void mta_swizzledMethod_3_bool(id self, SEL _cmd, BOOL arg) {
...
static void mta_swizzledMethod_4(id self, SEL _cmd, id arg, id arg2) {
	Method aMethod = class_getInstanceMethod([self class], _cmd);
	MTASwizzle *swizzle = (MTASwizzle *)[swizzles objectForKey:(__bridge id)((void *)aMethod)];
	if (swizzle) {
...
static void (*mta_swizzledMethods[MAX_ARGS - MIN_ARGS + 1])() = {mta_swizzledMethod_2, mta_swizzledMethod_3, mta_swizzledMethod_4};
static void (*mta_swizzledMethods_bool[MAX_BOOL_ARGS - MIN_BOOL_ARGS + 1])(id, SEL, BOOL) = {mta_swizzledMethod_3_bool};

@implementation MTASwizzler
...
+ (void)printSwizzles {
	...
	MTASwizzle *swizzle;
	while ((swizzle = (MTASwizzle *)[en nextObject])) {
		//        MTADebug(@"%@", swizzle);
	}
}

+ (MTASwizzle *)swizzleForMethod:(Method)aMethod {
	return (MTASwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
}
...
+ (void)setSwizzle:(MTASwizzle *)swizzle forMethod:(Method)aMethod {
...
+ (void)swizzleSelector:(SEL)aSelector
	...
	IMP swizzledMethod = (IMP)mta_swizzledMethods[numArgs - 2];
	[MTASwizzler swizzleSelector:aSelector onClass:aClass withBlock:aBlock andSwizzleMethod:swizzledMethod named:aName];
}
...
+ (void)swizzleBoolSelector:(SEL)aSelector
	...
	IMP swizzledMethod = (IMP)mta_swizzledMethods_bool[numArgs - 3];
	[MTASwizzler swizzleSelector:aSelector onClass:aClass withBlock:aBlock andSwizzleMethod:swizzledMethod named:aName];
}
...
+ (void)swizzleSelector:(SEL)aSelector
	...
	MTASwizzle *swizzle = [self swizzleForMethod:aMethod];
	...
	if (isLocal) {
		if (!swizzle) {
			...
			swizzle = [[MTASwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
		...
		MTASwizzle *newSwizzle = [[MTASwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
