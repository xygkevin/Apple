...
#define CR_LF_SIZE 2
	...
	while (true) {
		...
		for (; i + BINARY_UNIT_SIZE - 1 < lineEnd; i += BINARY_UNIT_SIZE) {
			...
			outputBuffer[j++] = (char)base64EncodeLookup[((inputBuffer[i] & 0x03) << 4) | ((inputBuffer[i + 1] & 0xF0) >> 4)];
			outputBuffer[j++] = (char)base64EncodeLookup[((inputBuffer[i + 1] & 0x0F) << 2) | ((inputBuffer[i + 2] & 0xC0) >> 6)];
...
//
+ (NSData *)mta_dataFromBase64String:(NSString *)aString {
	NSData *data = [aString dataUsingEncoding:NSASCIIStringEncoding];
	size_t outputLength;
	void *outputBuffer = mta_NewBase64Decode([data bytes], [data length], &outputLength);
...
//	receiver.
//
- (NSString *)mta_base64EncodedString {
	size_t outputLength = 0;
	char *outputBuffer =
		mta_NewBase64Encode([self bytes], [self length], false, &outputLength);
