...
@class TTKGCDAsyncReadPacket;
@class TTKGCDAsyncWritePacket;
@class TTKGCDAsyncSocketPreBuffer;
...
@interface TTKGCDAsyncSocket : NSObject

/**
 * TTKGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, TTKGCDAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means TTKGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means TTKGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, TTKGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, TTKGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of TTKGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a TTKGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * the method will do nothing, and the delegate will not be called.
 * 
 * If you pass a buffer, you must not alter it in any way while TTKAsyncSocket is using it.
 * After completion, the data returned in socket:didReadData:withTag: will be a subset of the given buffer.
 * That is, it will reference the bytes that were appended to the given buffer via
 * the method [NSData dataWithBytesNoCopy:length:freeWhenDone:NO].
**/
 ...
 * the method will do nothing (except maybe DDLog.log a warning), and the delegate will not be called.
 * 
 ...
 * This is due to the fact that TTKGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 *     So TTKGCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default TTKGCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct TTKGCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, TTKGCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by TTKGCDAsyncSocket,
...
/**
 * TTKGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(TTKGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(TTKGCDAsyncSocket *)sock didAcceptNewSocket:(TTKGCDAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(TTKGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol TTKGCDAsyncSocketDelegate
@optional
...
**/
- (void)socket:(TTKGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(TTKGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(TTKGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(TTKGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(TTKGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(TTKGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(TTKGCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the TTKGCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(TTKGCDAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socketDidSecure:(TTKGCDAsyncSocket *)sock;
...
**/
- (void)socket:(TTKGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
