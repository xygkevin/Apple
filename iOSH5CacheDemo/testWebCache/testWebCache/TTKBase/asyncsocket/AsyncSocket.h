...
@class TTKAsyncSocket;
@class TTKAsyncReadPacket;
@class TTKAsyncWritePacket;
...
@protocol TTKAsyncSocketDelegate
@optional
...
**/
- (void)onSocket:(TTKAsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (void)onSocketDidDisconnect:(TTKAsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(TTKAsyncSocket *)sock didAcceptNewSocket:(TTKAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(TTKAsyncSocket *)sock wantsRunLoopForNewSocket:(TTKAsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(TTKAsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(TTKAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(TTKAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(TTKAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(TTKAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(TTKAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(TTKAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(TTKAsyncSocket *)sock;
...
@interface TTKAsyncSocket : NSObject
{
	...
	TTKAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	TTKAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the TTKAsyncSocket instance is locked in
...
// When an incoming connection is accepted, TTKAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the TTKAsyncSocket instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an TTKAsyncSocket instance (e.g. in a dealloc method)
 ...
 * The performance can be much improved by allowing TTKAsyncSocket to read larger chunks at a time and
...
/**
 * When you create an TTKAsyncSocket, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the TTKAsyncSocket instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
