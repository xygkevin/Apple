...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `TTKFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<TTKFMResultSet>` - Represents the results of executing a query on an `TTKFMDatabase`.
 - `<TTKFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<TTKFMDatabasePool>` - A pool of `TTKFMDatabase` objects.
 - `<TTKFMStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `TTKFMDatabase` object and use it across multiple threads. Instead, use `<TTKFMDatabaseQueue>`.
...
@interface TTKFMDatabase : NSObject  {
...
/** Create a `TTKFMDatabase` object.
 
 An `TTKFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `TTKFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `TTKFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    TTKFMDatabase *db = [TTKFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    TTKFMDatabase *db     = [TTKFMDatabase databaseWithPath:dbPath];
 ...
 @return `TTKFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `TTKFMDatabase` object.
...
/** Execute single update statement
 ...
 This method executes a single SQL update statement (i.e. any SQL that does not return results, such as `UPDATE`, `INSERT`, or `DELETE`. This method employs [`sqlite3_prepare_v2`](http://sqlite.org/c3ref/prepare.html) and [`sqlite_step`](http://sqlite.org/c3ref/step.html) to perform the update. Unlike the other `executeUpdate` methods, this uses DDLog.logf-style formatters (e.g. `%s`, `%d`, etc.) to build the SQL. Do not use `?` placeholders in the SQL if you use this method.

 @param format The SQL to be performed, with `DDLog.logf`-style escape sequences.

 @param ... Optional parameters to bind to use in conjunction with the `DDLog.logf`-style escape sequences in the SQL statement.
 ...
 @note This method does not technically perform a traditional DDLog.logf-style replacement. What this method actually does is replace the DDLog.logf-style percent sequences with a SQLite `?` placeholder, and then bind values to that placeholder. Thus the following command
...
/** Execute single update statement
 ...
 This method executes a single SQL update statement (i.e. any SQL that does not return results, such as `UPDATE`, `INSERT`, or `DELETE`. This method employs [`sqlite3_prepare_v2`](http://sqlite.org/c3ref/prepare.html) and [`sqlite_step`](http://sqlite.org/c3ref/step.html) to perform the update. Unlike the other `executeUpdate` methods, this uses DDLog.logf-style formatters (e.g. `%s`, `%d`, etc.) to build the SQL.
...
/** Execute select statement
 ...
 Executing queries returns an `<TTKFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[TTKFMResultSet next]>`) from one record to the other.
 ...
 @return A `<TTKFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see TTKFMResultSet
 @see [`TTKFMResultSet next`](<[TTKFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (TTKFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (TTKFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (TTKFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (TTKFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (TTKFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(TTKFMDatabase *adb) {
        ...
        int rowCount = 0;
        TTKFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [TTKFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using TTKFMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `TTKFMStatement`, but rather with `<TTKFMDatabase>` and `<TTKFMResultSet>` only.
...
@interface TTKFMStatement : NSObject {
